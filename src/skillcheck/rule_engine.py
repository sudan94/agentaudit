...
Rules are declarative regex patterns loaded from ``src/skillcheck/rules/*.yml``
...
from skillcheck.models import Finding, ParsedFile, Severity, escape_snippet
from skillcheck.parsers.markdown import line_in_fence

_RULE_ID_RE = re.compile(r"^SC-[A-Z]{2,4}-\d{3}$")
...
def load_builtin_rules() -> list[Rule]:
    rules: list[Rule] = []
    pkg = resources.files("skillcheck") / "rules"
