...
from skillcheck.analyzers import get_analyzer_rules
from skillcheck.config import ConfigError, load_config
...
from skillcheck.rule_engine import RuleEngine, RuleLoadError
from skillcheck.scanner import scan as run_scan
...
def rules_command(
    ...
    table = Table(title=f"skillcheck rules ({len(engine.rules) + len(analyzer_rules)} loaded)", box=None, padding=(0, 2))
...
def version_command() -> None:
    """Print the skillcheck version."""
    _stdout.print(f"skillcheck {__version__}")
