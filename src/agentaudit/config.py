"""Project configuration: ``.agentaudit.yml`` plus inline suppressions."""
...
from agentaudit.discovery import glob_match
from agentaudit.models import Finding, Severity

CONFIG_FILENAME = ".agentaudit.yml"

_INLINE_SUPPRESS_RE = re.compile(
    r"agentaudit:\s*ignore\s+(?P<ids>(?:AA-[A-Z]{2,4}-\d{3}|all)(?:\s*,\s*AA-[A-Z]{2,4}-\d{3})*)",
    re.IGNORECASE,
)
